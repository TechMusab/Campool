...
router.get('/stats/:userId', auth, async (req, res) => {
	try {
		...
		const ridesTaken = await Ride.countDocuments({ passengers: userId });
		...
		const takenAgg = await Ride.aggregate([
			{ $match: { passengers: new mongoose.Types.ObjectId(userId) } },
		...
		const co2Agg = await Ride.aggregate([
			{ $match: { $or: [ { driverId: new mongoose.Types.ObjectId(userId) }, { passengers: new mongoose.Types.ObjectId(userId) } ] } },
